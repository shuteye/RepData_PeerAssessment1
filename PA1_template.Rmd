...
print(matrix(data=c("mean","median",round(m1,digits=2),m2),nrow=2,ncol=2,
             byrow=T), quote=F)
```
...
plot(m3,type="l",main="Average number of Steps at each 5 minute Interval",
     ...
     sub=paste("Maximum Step Average =", round(max(m3),digits=2),
                "steps: At Interval", names(subset(m3,m3==max(m3)))),
     axes=F)
...
```{r}
print(paste("The number of missing step measurements is",
            table(is.na(data$steps))[[2]],"out of",
            length(data$steps),"measurements"), quote=F)
```
...
```{r}
data1<-data.frame(replace(data$steps,is.na(data$steps),r1),
...
```{r, echo=FALSE}
data1$date<-as.Date(as.character(data$date),format="%Y-%m-%d",tz="")
```
...
print((matrix(data=c("mean","median",round(m4,digits=2),m5),nrow=2,ncol=2,
              byrow=T)),quote=F)
```
...
print(paste("Revised total of steps (bootstrap replacement):",
            sum(data1$steps.replace,na.rm=T)))
```
...
m6d<-with(d2$Weekday,tapply(steps.replace,INDEX=interval,FUN=mean,na.rm=T))      
m6w<-with(d2$Weekend,tapply(steps.replace,INDEX=interval,FUN=mean,na.rm=T))      
```
...
plot(m6w,type="l",main="Weekend",
     xlab ="Interval", ylab="Average number of Steps",
     sub=paste("Maximum Step Average =", round(max(m6w),digits=2),
                "steps: At Interval", names(subset(m6w,m6w==max(m6w)))),
     axes=F)
axis(side=1,at=seq(along.with=m6w),labels=names(m6w),tick=F)
axis(side=2, tick=F)

plot(m6d,type="l",main="Weekday",
     xlab ="Interval", ylab="Average number of Steps",
     sub=paste("Maximum Step Average =", round(max(m6d),digits=2),
                "steps: At Interval", names(subset(m6d,m6d==max(m6d)))),
     axes=F)
